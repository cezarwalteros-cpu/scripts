(() => {
  ...
  function hideElementAndParents(el) {
    if (!el) return;
  ...
  function setupCountry() {
    ...
    if (typeof window.countryStateInfo !== "undefined") {
      const countryName = Object.keys(window.countryStateInfo)[0] || CONFIG.defaultCountryName;
      ...
      if (countryField.tagName === "INPUT") {
        countryField.value = CONFIG.defaultCountryName;
      } else if (countryField.tagName === "SELECT") {
        countryField.innerHTML = "";
        countryField.add(new Option(CONFIG.defaultCountryName, CONFIG.defaultCountryName));
        countryField.selectedIndex = 0;
      }
    }

    hideElementAndParents(countryField);
  }

  function setupStateCity() {
    if (typeof window.countryStateInfo === "undefined") return;
    ...
    if (!stateSel || !citySel || !statesMap) return;
  ...
  function setupEmail() {
    ...
    if (email.previousElementSibling) {
