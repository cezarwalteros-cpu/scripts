...
;(function() {
  ...
  function enviarAbandono(razon) {
    ...
    if (navigator.sendBeacon) {
      try {
        if (navigator.sendBeacon(WEBHOOK_URL, new Blob([body], {type: 'application/json'}))) {
    ...
    // Fallback: XHR (instancia nueva, no modifica global)
    try {
      var x = new XMLHttpRequest();
      x.open('POST', WEBHOOK_URL, true);
      x.setRequestHeader('Content-Type', 'application/json');
      x.onload = function() { log(x.status >= 200 && x.status < 300 ? '✓ Enviado' : '✗ Error ' + x.status); };
      x.send(body);
