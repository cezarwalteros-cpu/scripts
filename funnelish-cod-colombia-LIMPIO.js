...
(function () {
  ...
  function ocultarCampo(nombre) {
    ...
    if (el) {
      ...
      if (formEl) {
        formEl.style.cssText = 'display: none !important; visibility: hidden !important; height: 0 !important; margin: 0 !important; padding: 0 !important; overflow: hidden !important;';
      }
    }
  }
  ...
  function validarTelefonoEnClick() {
    document.addEventListener('click', function(e) {
      ...
      if (!fmt) {
        ...
        phone.value = fmt;
        limpiarError(phone);
        // Permitir que continúe el submit
  ...
  function obtenerDatosFormulario() {
    ...
    if (ciudadInput && ciudadInput.value) datos.ciudad = ciudadInput.value.trim();

    var deptoSelect = document.querySelector('select[name="shipping_state"]');
    if (deptoSelect && deptoSelect.value) datos.departamento = deptoSelect.value;
  ...
  function validarDatosMinimos(datos) {
    if (!datos.nombre || datos.nombre.length < 2) return false;
    if (!datos.telefono || datos.telefono.length < 10) return false;
    if ((!datos.productos || datos.productos.length === 0) && datos.total === 0) return false;
    return true;
  }
  ...
  function enviarWebhook(url, datos, tipo) {
    ...
    .then(function(response) {
      if (response.ok) {
        log('Webhook ' + tipo + ' enviado', 'success');
        return response.json();
      }
      throw new Error('Error HTTP: ' + response.status);
    })
    .catch(function(error) {
      log('Error webhook ' + tipo + ': ' + error.message, 'error');
    });
  }

  function enviarPedidoCompleto() {
    if (window.__pedidoCompleto) return;
  ...
  function initCore() {
    ...
    log('✅ Script inicializado', 'success');
  }
