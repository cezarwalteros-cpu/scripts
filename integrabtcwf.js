(function(){
    ...
    // ========== CONFIGURACIÓN CARRITO ABANDONADO ==========
    var webhookCarritoAbandonado = 'https://programacioncwf.app.n8n.cloud/webhook/funnelish-evento-parcial';
    var nombreCapturado = '';
    var telefonoCapturado = '';
    var dioClickEnComprar = false;
    // ========== FIN CONFIGURACIÓN ==========
    ...
    // ========== VALIDACIÓN TELÉFONO COLOMBIA (E.164: +57##########) ==========
    ...
    function validarYMostrarEstadoTelefono(input) {
        ...
        if (validarTelefonoColombia(valor)) {
            ...
            telefonoCapturado = extraerDigitosColombia(valor);
            // =================================================================
    ...
    function init() {
        ...
        // ========== SETUP TELÉFONO CON VALIDACIÓN ESTRICTA ==========
        ...
        // ========== SETUP CAPTURA NOMBRE PARA CARRITO ABANDONADO ==========
        var nombre = document.querySelector('input[name="first_name"], input[name="shipping_first_name"]');
        if (nombre) {
            nombre.addEventListener('input', function() {
                nombreCapturado = this.value.trim();
            });
    ...
    // ========== VALIDACIÓN FINAL EN SUBMIT (BLOQUEA SI INVÁLIDO) ==========
    document.addEventListener('submit', function(e) {
        ...
        dioClickEnComprar = true;
        // =====================================================
        
    }, true);
    // ========== FIN VALIDACIÓN SUBMIT ==========
    
    // ========== CARRITO ABANDONADO - ENVIAR AL CERRAR PÁGINA ==========
    window.addEventListener('beforeunload', function() {
        // Solo enviar si:
        // 1. Tiene nombre
        // 2. Tiene teléfono válido (10 dígitos)
        // 3. NO dio click en comprar/submit
        
        if (nombreCapturado && telefonoCapturado.length >= 10 && !dioClickEnComprar) {
            ...
            var datos = {
                body: {
                    ...
                    first_name: nombreCapturado,
                    phone: telefonoCapturado,
                    city: ciudad ? ciudad.value : '',
                    address: direccion ? direccion.value : '',
                    email: email ? email.value : '',
                    product_name: document.querySelector('.product-name, [data-product-name], .product-title, h1')?.textContent?.trim() || 'Producto',
                    page_url: window.location.href
                }
            };
            
            // Enviar con sendBeacon (funciona al cerrar página)
            navigator.sendBeacon(webhookCarritoAbandonado, JSON.stringify(datos));
        }
    });
    // ========== FIN CARRITO ABANDONADO ==========
