...
// Bloquear submit si el teléfono no es válido o el nombre está incompleto
    ...
    btnSubmit.addEventListener('click', function (event) {
        ...
        if (inputTel) {
            ...
            if (!telValido) {
                ...
                mostrarErrorCampo(inputTel, 'Ingresa un número válido (10 dígitos, empieza con 3). Ej: 3001234567');
                bloqueado = true;
            } else {
        ...
        if (inputNombre) {
            if (!nombreEsCompleto(inputNombre.value)) {
                console.log('[BienestarTotal] Submit bloqueado — se requiere nombre y apellido');
                inputNombre.style.borderColor = '#e53e3e';
                mostrarErrorCampo(inputNombre, 'Por favor ingresa tu nombre y apellido completos');
                bloqueado = true;
            } else {
...
function mostrarErrorCampo(input, mensaje) {
    const errorId = 'error-' + (input.name || input.id || 'campo');
    let div = document.getElementById(errorId);
    if (!div) {
...
function quitarErrorCampo(input) {
    const errorId = 'error-' + (input.name || input.id || 'campo');
...
function nombreEsCompleto(nombre) {
...
function datosListosParaAbandonado() {
    ...
    const telValido    = /^3\d{9}$/.test(digits);
    const nombreValido = nombreEsCompleto(datos.cliente.nombreCompleto);

    if (!nombreValido) console.log('[BienestarTotal] Carrito abandonado no enviado — se requiere nombre y apellido');
...
// El webhook de pedido completo lo maneja Funnelish nativamente.
// Aquí solo marcamos pedidoConfirmado = true para bloquear
// el carrito abandonado una vez el cliente hace submit exitoso.
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    btnSubmit.addEventListener('click', function () {
        ...
        const digits = inputTel.value.replace(/\D/g, '');
        const telValido = /^3\d{9}$/.test(digits) || /^\+573\d{9}$/.test(inputTel.value);
        if (!telValido) return;

        // Marcar como confirmado → bloquea carrito abandonado
        pedidoConfirmado = true;
        console.log('[BienestarTotal] 🔒 Submit detectado — pedidoConfirmado = true');

        // Guardar en localStorage para detectar pedido doble en visitas futuras
        const datos = recolectarDatos();
