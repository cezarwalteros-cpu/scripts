/**
 * BIENESTAR TOTAL CUSTOM CODE
 * Versión: Autónoma (Sin dependencias externas)
 * ID de Rastreo: idBtotal
 */

const HOST_WEBHOOK = ""; // URL Final
const HOST_PREELIMINAR_WEBHOOK = "https://integration.toxolinko.site/webhook/BienestarTotalUpPreliminar";
const STORE_NAME = "BienestarTotal";

// --- BASE DE DATOS DE UBICACIONES COLOMBIA ---
...
function establecerCountry(country, countrySel) {
    ...
    if (countrySel.tagName === "INPUT") {
        ...
        countrySel.add(newOption, 1);
        countrySel.selectedIndex = 1;
    }
}
...
// --- LÓGICA DE TELÉFONO Y VALIDACIÓN ---
...
function verifyPhone() {
    ...
    div.innerHTML = `<input id="inputPhoneInvalid" class="invalid" placeholder="${mensajeAdvertencia}" style="border: 2px solid #f89708 !important; display: none; font-size: 14px; text-align: left; font-weight: normal; font-style: normal;" readonly="true">`;
    inputTelefono.parentNode.insertBefore(div, inputTelefono.nextSibling);

    inputTelefono.addEventListener('input', function() {
        let raw = inputTelefono.value.toString();
        let digits = raw.replace(/[^0-9]/g, '');
        if (digits.startsWith('57')) digits = digits.substring(2);
        if (!digits.startsWith('3')) digits = '';
        if (digits.length > 10) digits = digits.slice(0, 10);
        inputTelefono.value = '+57' + digits;
    });
}

// Bloqueo de envío si el teléfono es inválido
document.addEventListener('DOMContentLoaded', function () {
    ...
    submitButton.addEventListener('click', function (event) {
        ...
        if (!patronTelefonoColombia.test(telefono)) {
            ...
            pushSendLead(); // Enviar lead final
        }
    }, true);
});

// --- LÓGICA DE RASTREO IDBTOTAL ---
let idBtotal;
function setIdLocalStorage() {
    if (idBtotal) return { id: idBtotal };
    const currentUrl = `${window.location.hostname}${window.location.pathname}`;
    let elementosArray = JSON.parse(localStorage.getItem('idBtotal')) || [];
    let existingElement = elementosArray.find(e => e.url === currentUrl && new Date() - new Date(e.date) < 20 * 60 * 1000);
    if (existingElement) {
        idBtotal = existingElement.id;
        return existingElement;
    }
    const newElement = {
        url: currentUrl,
        id: Array.from({ length: 10 }, () => 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789'.charAt(Math.floor(Math.random() * 62))).join(''),
        date: new Date().toISOString()
    };
    elementosArray.push(newElement);
    localStorage.setItem('idBtotal', JSON.stringify(elementosArray));
    idBtotal = newElement.id;
    return newElement;
}

window.addEventListener('load', () => {
    setIdLocalStorage();
    const idField = document.querySelector(`[data-name="idBtotal"]`);
    if (idField) {
        idField.value = JSON.stringify({ idBtotal: idBtotal, urlOrigin: `${location.hostname}${location.pathname}` });
        idField.parentElement.style.display = 'none';
    }
    // Limpieza de campo Dirección autocompletado erróneamente
    const DireccionInput = document.querySelector(`[data-name="Direccion"]`);
    if (DireccionInput) DireccionInput.value = DireccionInput.value.replace(/{.*}/, '');
});

// --- LÓGICA DE ENVÍO A WEBHOOKS (INTEGRACIÓN) ---
let timerId;
const timeDebouncing = 4000;

window.addEventListener('load', () => {
    document.body.addEventListener('input', () => {
        clearTimeout(timerId);
        timerId = setTimeout(() => pushLead(), timeDebouncing);
    });
});

async function pushLead() {
    const data = recoverData();
    if (data.client.phone === '' || data.client.phone === '+57') return;
    await sendLead(data, true);
}

async function pushSendLead() {
    const data = recoverData();
    await sendLead(data, false);
}
...
function recoverData() {
    ...
    return {
        ...
        dataAddress: {
            ...
            city: getValue('shipping_city'),
            state: getValue('shipping_state'),
            country: getValue('shipping_country'),
...
window.addEventListener('load', () => {
    ...
    if (camposCustom.length >= 3 && direccion) {
        const updateDir = () => {
            direccion.value = `${camposCustom[0].value} , ${camposCustom[1].value} , ${camposCustom[2].value}`;
        };
        direccion.setAttribute('readonly', true);
        camposCustom[0].addEventListener('input', updateDir);
        camposCustom[1].addEventListener('input', updateDir);
        camposCustom[2].addEventListener('input', updateDir);
    }
});
